...
def buscar_y_actualizar():
    try:
        # Importar archivos
        archivo_items = filedialog.askopenfilename(title="Seleccionar archivo Items", filetypes=[("Archivos Excel", "*.xlsx")])
        archivo_propuesta = filedialog.askopenfilename(title="Seleccionar archivo Propuesta", filetypes=[("Archivos Excel", "*.xlsx")])

        if not archivo_items or not archivo_propuesta:
            messagebox.showerror("Error", "¡Ambos archivos deben ser seleccionados!")
            return

        # Cargar archivos Excel con manejo de errores
        try:
            items_df = pd.read_excel(archivo_items)
        ...
        print("Columnas en archivo Items:", items_df.columns)
        print("Columnas en archivo Propuesta:", propuesta_df.columns)
...
root = tk.Tk()
root.title("Herramienta de Actualización de Productos")

frame = tk.Frame(root, padx=20, pady=20)
frame.pack()

label = tk.Label(frame, text="Haz clic en el botón para comenzar a procesar los archivos:")
label.pack(pady=10)

boton = tk.Button(frame, text="Comenzar", command=buscar_y_actualizar)
boton.pack()
